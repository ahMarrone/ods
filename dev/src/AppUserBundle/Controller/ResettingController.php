...
class ResettingController extends Controller
{
    ...
    public function requestAction(Request $request)
    {
        ...
        $idUserToReset = intval($request->get('id'));
        if ($idUserToReset != 0) {
            if ($currentUser !== null) {
                if ($currentUser->getId() != $idUserToReset) {
                $this->denyAccessUnlessGranted('ROLE_SUPER_ADMIN', $this->getUser(), '¡No tiene permisos para ingresar a esta página!');
                }
                $em = $this->getDoctrine()->getManager();
                $userToReset = $em->getRepository('AppBundle:Usuarios')->findOneById($idUserToReset);    
            } else {
                throw new NotFoundHttpException('No es posible recuperar el objeto solicitado');
            }
    ...
    public function sendEmailAction(Request $request)
    {
        ...
        // $username = $data["fos_user_resetting_form"]["username"];

        /** @var $user UserInterface */
        $user = $this->get('fos_user.user_manager')->findUserByUsernameOrEmail($usermail);
        if (null !== $user) {
